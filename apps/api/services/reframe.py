...
def build_reframe_track(width:int,height:int,duration:float,points:list[dict]|None=None)->list[dict]:
    ...
    if points:
        return [{"time":float(p["time"]),"x":float(p["x"]),"y":float(p["y"]),"confidence":float(p.get("confidence",1))} for p in points]
    cx=crop["x"]+crop["width"]/2; cy=crop["y"]+crop["height"]/2
    return [{"time":0.0,"x":cx,"y":cy,"confidence":1.0},{"time":float(duration),"x":cx,"y":cy,"confidence":1.0}]