def _merge_windows(windows: list[dict], gap: float = 0.12) -> list[dict]:
    if not windows:
        return []
    items = sorted(windows, key=lambda x: x["start"])
    out = [dict(items[0])]
    for w in items[1:]:
        if w["start"] - out[-1]["end"] <= gap:
...
def build_plan(
    ...
    cuts = sorted(
        set(
            [0.0, duration]
            + [float(x) for x in (scene_cuts or []) if 0 < float(x) < duration]
        )
    )
    ...
    index = 0
    for w in source:
        cursor = float(w["start"])
        end_limit = float(w["end"])
        while cursor < end_limit - 0.05:
            ...
            candidates = [c for c in cuts if cursor + min_clip <= c <= target + 0.15]
                ...
                min(candidates, key=lambda c: abs(c - target)) if candidates else target
            )
            if beat is not None and abs(beat - target) < 0.45:
                end = beat
            ...
            if end - cursor >= 0.5:
                nearby_words = [
                    x
                    for x in (words or [])
                    if float(x.get("start", 0)) < end
                    and float(x.get("end", 0)) > cursor
                ]
                ...
                segments.append(
                    {
                        ...
                        "zoom": (
                            1.06
                            if preset in {"viral", "energy"} and index % 3 == 0
                            else 1.0
                        ),
                        "speech_density": round(density, 3),
                        "beat_sync": beat_sync,
                        "beat": round(beat, 3) if beat is not None else None,
                        "reason": "silence-aware, scene-aware, beat-aligned pacing",
                    }
                )
                index += 1
            cursor = end
