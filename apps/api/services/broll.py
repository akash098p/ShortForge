...
def make_broll_plan(segments:list[dict],assets:list[str],coverage:float=0.28)->list[dict]:
    if not assets:return segments
    out=[]; ai=0
    for i,s in enumerate(segments):
        ...
        x=dict(s)
        x["broll"]={"asset":assets[ai%len(assets)],"start":round(start,3),"end":round(start+length,3),"mode":"cutaway"}
        ai+=1; out.append(x)
    return out

def render_broll(source:str,asset:str,output:str,start:float,end:float)->None:
    ...
    try:
        ...
        subprocess.run(["ffmpeg","-y","-ss",str(start),"-i",asset,"-t",str(dur),"-vf",vf,"-an","-c:v","libx264","-preset","medium","-crf","18","-pix_fmt","yuv420p",str(fg)],check=True)
