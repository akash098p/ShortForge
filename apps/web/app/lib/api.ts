...
export async function renderEditPlan(input:{sourcePath:string;segments:any[];captions:any[];reframe:any[];preset:string}){
 const filename=`short-${Date.now()}.mp4`;
 const r=await fetch(API_BASE+"/v1/render-plan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source_path:input.sourcePath,output_path:"shortforge-render/"+filename,segments:input.segments,captions:input.captions,reframe:input.reframe,preset:input.preset})});
 if(!r.ok) throw new Error(await r.text());
 const data=await r.json(); return {...data,preview_url:API_BASE+"/outputs/"+filename};
}
