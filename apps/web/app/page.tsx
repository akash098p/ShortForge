...
export default function Home(){
 ...
 const renderShort=async()=>{if(!plan||!sourcePath)return;setStatus("rendering");setError(null);try{const result=await renderEditPlan({sourcePath,segments:plan.segments||[],captions:plan.captions||[],reframe:plan.reframe||[],preset});setRendered(result.preview_url);setStatus("rendered")}catch(e){setStatus("error");setError(e instanceof Error?e.message:"Render failed")}};
