...
import {createEditPlan,uploadVideo} from "./lib/api";
...
export default function Home(){
 ...
 const autoEdit=async()=>{if(!meta||!file||!sourcePath)return;setStatus("analyzing");setError(null);try{await createEditPlan({sourceName:file.name,sourcePath,...meta,preset});setStatus("complete")}catch(e){setStatus("error");setError(e instanceof Error?e.message:"Analysis failed")}};
 ...
 return <main className="shell">
  ...
  <input ref={input} hidden type="file" accept="video/*" onChange={e=>onFile(e.target.files?.[0])}/>{error&&<div className="meta">{error}</div>}{meta&&<div className="meta">{Math.round(meta.duration*10)/10}s · {meta.width}×{meta.height} · 9:16 crop {crop?.width}×{crop?.height}</div>}</div>
