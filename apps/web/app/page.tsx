...
export default function Home(){
 const input=useRef<HTMLInputElement>(null); const [video,setVideo]=useState<string|null>(null); const [file,setFile]=useState<File|null>(null); const [meta,setMeta]=useState<any>(null); const [preset,setPreset]=useState("viral"); const [status,setStatus]=useState("idle");
 ...
 const autoEdit=async()=>{if(!meta||!file)return;setStatus("analyzing");await new Promise(r=>setTimeout(r,700));setStatus("complete")};
 ...
 return <main className="shell">
  ...
  <div className="actions"><button className="primary" onClick={()=>input.current?.click()}><Upload size={18}/> {file?"Replace video":"Upload video"}</button>{file&&<button className="secondary" onClick={autoEdit}>{status==="analyzing"?<><Loader2 className="spin" size={17}/> Analyzing…</>:status==="complete"?<><CheckCircle2 size={17}/> Edit plan ready</>:<><Sparkles size={17}/> Auto Edit</>}</button>}</div>
